    ...
    public class SimulationFactory
    {
        ...
        {            
            ...
            Task retroLayerTask = Task.Factory.StartNew(() =>
            {
                ...
                using (StreamWriter sw = new StreamWriter(fs))
                {
                    sw.WriteLine($"CALCULATIONID,RETROLEVEL,RETROPROGRAMID,RETROPROGRAMTYPE,RESETTYPE,RETROINCEPTION,RETROEXPIRATION,LAYERID,LAYERINCEPTION,LAYEREXPIRATION,LAYERSTATUS,CESSIONSTARTDATEINCLUSIVE,CESSIONENDDATEEXCLUSIVE,CURRENCY,DEPOSITPREMIUM,SUBJECTPREMIUM,SUBJECTPREMIUMPLACED,CEDEDPREMIUM,DEPOSITLIMIT,SUBJECTLIMIT,SUBJECTLIMITPLACED,CEDEDLIMIT,GROSSCESSIONAFTERPLACEMENT,NETCESSION");
                    foreach (RetroLayerMetrics m in retroSummaryMetrics.RetroLayerMetrics)
                    {
                        sw.WriteLine($"{calculationId},{m.RetroLevel},{m.RetroProgramid},{m.RetroProgramType.ToString()},{resetType.ToString()},{m.RetroInceptionDate.ToString("yyyy-MM-dd")},{m.RetroExpirationDate.ToString("yyyy-MM-dd")},{m.LayerId},{m.LayerInceptionDate.ToString("yyyy-MM-dd")},{m.LayerExpirationDate.ToString("yyyy-MM-dd")},{m.LayerStatus.ToString()},{m.StartDateInclusive.ToString("yyyy-MM-dd")},{m.EndDateInclusive.ToString("yyyy-MM-dd")},{baseCurrency},{m.DepositPremium},{m.SubjectPremium},{m.SubjectPremiumPlaced},{m.CededPremium},{m.DepositLimit},{m.SubjectLimit},{m.SubjectLimitPlaced},{m.CededLimit},{m.GrossCessionAfterPlacement},{m.NetCession}");
                    }
                    sw.Flush();
                }
