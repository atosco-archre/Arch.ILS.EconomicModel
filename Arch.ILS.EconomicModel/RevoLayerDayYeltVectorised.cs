...
{
    public unsafe class RevoLayerDayYeltVectorised : IYelt, IDisposable
    {
                    ...
                    foreach (var entry in dayEntries)
                    {
                        if ((currentInBufferIndex % BUFFER_ITEM_COUNT) == 0)
                        {
                            if (++currentBuffer == _lastBufferIndex)
                            {
                                ...
                                currentYearDayPerilIdEventIdPtr = (long*)_dayYearPerilIdEventIdKeys[currentBuffer].ToPointer();
        ...
        public RevoLayerDayYeltVectorised(in string filePath, bool parallelise = false)
        {
            if (parallelise)
