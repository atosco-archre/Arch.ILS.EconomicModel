...
{
    public unsafe class RevoLayerYeltStandardUnsafe : IBenchmarkYelt, IDisposable
    {
        ...
        public RevoLayerYeltStandardUnsafe(in int lossAnalysisId, in int layerId, in IEnumerable<RevoLayerYeltEntry> yelt)
        {
            _disposed = false;
            ...
            while (Unsafe.IsAddressLessThan(ref spanStart, ref spanEnd))
            {
                if (spanStart != null)
                {
                    ...
                    *distinctYears++ = *spanStart.First().GetYear();
                    *yearRepeatCount++ = spanStart.Count;//yearEndIndexExclusive
