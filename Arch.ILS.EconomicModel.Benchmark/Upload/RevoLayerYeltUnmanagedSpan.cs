...
namespace Arch.ILS.EconomicModel.Benchmark
{
    public unsafe class RevoLayerYeltUnmanagedSpan : IBenchmarkYelt, IDisposable
    {
        ...
        public RevoLayerYeltUnmanagedSpan(in int lossAnalysisId, in int layerId, in IEnumerable<RevoLayerYeltEntry> yelt)
        {
            ...
            while (Unsafe.IsAddressLessThan(ref spanStart, ref spanEnd))
            {
                if (spanStart != nint.Zero)
                {
                    ...
                    currentYearBufferSpan[currentYearInBufferIndex] = *yearEntries.First().GetYear();
                    currentYearRepeatCountBufferSpan[currentYearInBufferIndex] = yearEntries.Count;//yearEndIndexExclusive
        ...
        public ReadOnlySpan<short> DistinctYears(in uint i) => new ReadOnlySpan<short>(_distinctYears[i], 0, i == _lastYearBufferIndex ? _lastYearBufferItemCount : BUFFER_ITEM_COUNT);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ReadOnlySpan<int> YearRepeatCount(in uint i) => new ReadOnlySpan<int>(_yearRepeatCount[i], 0, i == _lastYearBufferIndex ? _lastYearBufferItemCount : BUFFER_ITEM_COUNT);
